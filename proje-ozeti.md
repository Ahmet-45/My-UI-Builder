AGNOSTIC UI BUILDER - PROJE DÖKÜMANTASYONU VE KAYNAK KODLAR

**[SİSTEM BAĞLAMI / CONTEXT]**
Bu döküman, sıfırdan geliştirilen bir "Arayüz İnşa Aracı"nın (UI Builder) güncel durumunu içerir. 
- **Temel Felsefe:** "Agnostic Core". Sistem arka planda çıktıdan bağımsız saf bir JSON ağacı (Abstract Syntax Tree benzeri) oluşturur. 
- **Mühendislik Prensipleri:** "Separation of Concerns" (Sorumlulukların Ayrılığı), Immutability (Değişmezlik) ve Vanilla yaklaşımı.
- **Kullanılan Teknolojiler:** React (TypeScript), Zustand (Atomic State & Selectors), Vanilla CSS/HTML jeneratörleri.

Aşağıda projenin tüm veri tipleri, store yapısı, yardımcı fonksiyonları ve React bileşenleri yer almaktadır. Lütfen analizlerinizi bu mimari kararları göz önünde bulundurarak yapın.

Merhaba Claude! Seninle bir 'Senior Software Architect' ve 'Socratic Mentor' olarak çalışmak istiyorum. Lütfen bana asla doğrudan kopyala-yapıştır tam çözümler verme, bunun yerine bana mantığı, mimariyi ve algoritmaları açıkla; kodu benim yazmamı sağla.

Şu anda React (Vite) ve Zustand kullanarak, harici ağır kütüphanelerden (uuid, styled-components vb.) bağımsız bir 'Agnostic UI Builder' geliştiriyorum. Ek'teki .md dosyasında projenin tüm mimarisi, felsefesi ve güncel kodları yer alıyor.

Şu anki Mimari Sorunum:
PreviewPanel içinde, ürettiğimiz saf HTML string'ini dangerouslySetInnerHTML ile ekrana basıyoruz. Ancak bu saf HTML olduğu için ekrandaki kutulara tıklayıp bir elemanı 'seçemiyorum' (Zustand'daki selectedNodeId hep null kalıyor). Bu yüzden LeftPanel üzerinden yeni bir kutu veya buton eklemek istediğimde 'Lütfen bir eleman seçin' hatasına takılıyorum.

Senden Beklentilerim:

Eklediğim dosyayı inceleyip bana kısa, profesyonel bir Code Review (Kod İncelemesi) yapman. Eksik veya anti-pattern gördüğün yerleri belirtmen.

Bu 'seçememe' (selection) sorununu aşmak için genel mimaride nasıl bir revizyona gitmemiz gerektiğini anlatman. (Örneğin; saf HTML export işlemine dokunmadan, araya tıklanabilir React bileşenlerinden oluşan bir 'Canvas / Render' katmanı mı eklemeliyiz? Yoksa LeftPanel mantığını mı değiştirmeliyiz?)

Çözüm yolunu bana ipuçları ve ufak iskelet kodlar (scaffolding) vererek adım adım uygulattırman.

Dosyayı inceledikten sonra bana sadece 'Projeyi anladım' de ve ilk mimari yönlendirmen ile başla.
...
export const LeftPanel = () => {
  ...
  const handleAddElement = (type: "container" | "button") => {
    if (selectedNodeId === null){
      alert("Lutfen once bir eleman secin");
      return;
    }
  ...
  return (
    <div style={{ width: '200px', padding: '16px', backgroundColor: '#f4f4f4'}}>
      <h3>Arac Kutusu</h3>
      <button onClick={() => handleAddElement("container")}>
        Kutu Ekle
      </button>
...
import { updateNodeProps, insertNode } from '../utils/treeHelpers';
...
interface BuilderStore {
  ...
  addNode: (parentId: string, nodeType: "container" | "button") => void;
}
...
export const useBuilderStore = create<BuilderStore>((set, get) => ({
  ...
  addNode: (parentId, nodeType) => {
    ...
    const newTree = insertNode(state.tree, parentId, newNode);
    set({ tree: newTree });
  }
}));
...
export function updateNodeProps(node: UINode, targetId: string,    newProps: Partial<NodeProps>): UINode {
  ...
  if(node.children && node.children.length > 0){
    return {
      ...node,
      children: node.children.map(child => updateNodeProps(child, targetId, newProps))
    };
  }
  return node;
}
...
export function insertNode(currentNode: UINode, parentId: string, newNode: UINode): UINode {
  ...
  if(currentNode.children && currentNode.children.length > 0){
    return {
      ...currentNode,
      children: currentNode.children.map(child => insertNode(child,parentId, newNode))
    };
  }
  return currentNode;
}
...
import { previewPanel as PreviewPanel } from "./components/panel/PreviewPanel";
...
<html lang="en">
  <head>
    ...
    <title>Agnostic UI Builder</title>
  </head>
  <body>
    <div id="root"></div>
